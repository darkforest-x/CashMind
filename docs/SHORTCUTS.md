...
`YOUR_SHORTCUT_TOKEN` 来自 VPS 的 `/var/www/cashmind/.env`。现在推荐使用设置页生成的“万能入口 URL”，token 会封装在 URL 里，快捷指令里不需要再手动配置 Header。

万能入口：

```text
POST http://103.214.174.58:3000/api/shortcut/capture?token=YOUR_SHORTCUT_TOKEN
```
...
```bash
curl -X POST "http://103.214.174.58:3000/api/shortcut/capture?token=YOUR_SHORTCUT_TOKEN" \
